const { saveUser, getUser, updateUser } =
  require("../../models/index").userModule;
...
module.exports = {
  ...
  async signup(req, res) {
    ...
    if (!Phone_Number)
      return errorResponse(res, 422, "Phone_Number is required");
    ...
    try {
      ...
      if (!USER_DATA) {
        ...
        sendMail(
          data,
          process.env.LOGIN_OTP_TEMPLATE,
          "Signup Verification Code"
        );
        return succesResponse(
          res,
          200,
          "you have signup successfully! now you can verify your email."
        );
      } else {
  ...
  async login(req, res) {
    ...
    try {
      ...
      if (!USER_DATA)
        return errorResponse(res, 422, "Please enter a valid email");
      const RESULT = bcryptVerify(req.body.Password, USER_DATA.Password);
      if (!RESULT)
        return errorResponse(res, 422, "Entered password is not matched");
      const payload = {
  ...
  async forgotPassword(req, res) {
    ...
    try {
      ...
      return succesResponse(
        res,
        200,
        "Password verification mail send to your given email!"
      );
    } catch (err) {
      return errorResponse(res, 422, err.message);
    }
  ...
  async resetPassword(req, res) {
    ...
    if (!ACCESS_TOKEN)
      return errorResponse(res, 422, "accessToken is required!");
    if (!req.body.Password)
      return errorResponse(
        res,
        422,
        "Please provide a Password for reset your password."
      );
