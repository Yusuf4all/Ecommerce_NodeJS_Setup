...
const { getSupplier, saveSupplier, updateSupplier } =
  require("../../models").supplierModule;
...
module.exports = {
    ...
    try {
      ...
      if (!SUPPLIER) {
        ...
        return errorResponse(
          res,
          422,
          `${req.body.Email} is already registerd please try another!`
        );
      }
    } catch (err) {
      console.log(err);
  ...
  async update(req, res) {
    try {
      if (req.body.Email)
        return errorResponse(res, 422, "Here you can not update your email!");
      if (req.body.Stripe_Account)
        return errorResponse(
          res,
          422,
          "Here you can not update Stripe_Account!"
        );
      if (req.body.Password) {
        req.body.Password = await bcryptHash(req.body.Password, 10);
      }
